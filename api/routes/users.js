...
router.get("/authenticate", verify, (req, res) => {
  User.findById(req.user._id)
    .select("-password") // use userid stored on jwt as payload
...
router.get("/org/:org_id", verify, (req, res) => {
  User.find({ orgId: req.params.org_id })
    .select("-password")
    .then((user) => res.json(user))
    .catch((err) => res.status(400).json("Error: " + err));
});
...
router.post("/add-new-user", async (req, res) => {
  const { name, email, password, organizationId, role, position } = req.body;
  ...
  if (!user) {
    ...
    try {
      ...
      res.status(200).json({
        status: "success",
        message: `Added user ${name} successfully.`,
        userId: user._id,
      });
    } catch (err) {
